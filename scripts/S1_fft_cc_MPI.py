...
local_data_path = os.path.join(
    rootpath, "2004_*"
)  # absolute dir where SAC files are stored: this para is VERY IMPORTANT and has to be RIGHT if input_fmt is not asdf!!!
locations = os.path.join(
    rootpath, "station.txt"
)  # station info including network,station,channel,latitude,longitude,elevation: only needed when input_fmt is not asdf
...
for ick in range(rank, splits, size):
    ...
    for ista in range(len(sta_list)):
        ...
        for itag in range(len(all_tags)):
            ...
            trace_stdS, dataS_t, dataS = noise_module.cut_trace_make_statis(
                fc_para, source
